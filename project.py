...
class  MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.capture_thread = CaptureThread(self.conf, self.model, self.conf['gpu_model'] if self.conf['use_gpu'] else 'cpu', self.image_label)
...
class CaptureThread(QThread):
    def __init__(self, conf, model, device, image_label):
        super().__init__()
        self.conf = conf
        self.model = model
        self.device = device
        ...
        while True:
            ...
            if self.conf['show_fps']:
                ...
                if self.elapsed_time >= 1.0:
                    ...
                    self.frame_count = 0
                    self.start_time = time.time()
                cv2.putText(image, f'FPS: {self.frame_rate:.2f}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
